...
import { IPaymentHistoryData } from '@/types/companyInterface';
import { IWalletLedger } from '@/types/fundManagementInterface';


type Props = {
    columns: any;
    dataList: IWalletLedger[];
    welcomePage?: boolean;
    searchQuery:string
}

const CopmanyWalletHistoryTable = (props: Props) => {
    ...
    const walletLedgerHistoryLoader = useSelector((state: AppState) => state.company.walletLedgerHistoryLoader)
     const walletLedgerHistoryMetaParams = useSelector((state: AppState) => state.company.walletLedgerHistoryMetaParams)
    ...
    useEffect(() => {
        if (
            inView === true &&
            paymentHistoryLength < walletLedgerHistoryMetaParams?.meta_params.total_count &&
            walletLedgerHistoryMetaParams?.meta_params?.hasMorePage === true &&
            Number(pageRequested) !== Number(walletLedgerHistoryMetaParams?.meta_params?.nextPage)
        ) {
            setPaymentHistoryParams((candidateParams) => {

                return { ...candidateParams, page: walletLedgerHistoryMetaParams?.meta_params.nextPage };
            });
            setScrollValue(true);
        }
    }, [inView, walletLedgerHistoryMetaParams?.meta_params.total_count, walletLedgerHistoryMetaParams?.meta_params?.hasMorePage]);
    ...
    useEffect(() => {
        if (
            ...
            setPageRequested(walletLedgerHistoryMetaParams?.meta_params.nextPage);
            let reqObj = {
    ...
    }, [walletLedgerHistoryMetaParams?.meta_params?.hasMorePage, walletLedgerHistoryMetaParams?.meta_params.nextPage, paymentHistoryParams.page, dispatch,
...
export default CopmanyWalletHistoryTable;
