...
import { downloadInvoice, getCompanyPaymentHistory } from '@/redux/actions/comanyUserActions';
...
const CopmanyPaymentHistoryTable = (props: Props) => {
    ...
    const invoiceDownloadLoading = useSelector((state: AppState) => state.company.invoiceDownloadLoading)
    const paymentHistoryMetaParams = useSelector((state: AppState) => state.company.paymentHistoryMetaParams)
                    ...
                    <table {...getTableProps()} className='upcoming_appoinments_table'>
                        ...
                        <tbody {...getTableBodyProps()}>
                            <>
                                ...
                                {rows.slice(0, props?.welcomePage ? 4 : rows.length).map((row: Row<IPaymentHistoryData>, index: number) => {
                                    prepareRow(row);
                                    return (
                                        <>
                                            <tr {...row.getRowProps()}>
...
export default CopmanyPaymentHistoryTable;
